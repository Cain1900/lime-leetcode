...
/**
 * Created by YuHang on 2023/12/24.
 */
public class Week20231224 {
    ...
    public int maximizeSquareArea(int m, int n, int[] hFences, int[] vFences) {
        PriorityQueue<Integer> hQueue = getPriorityQueue(m, hFences);
        PriorityQueue<Integer> vQueue = getPriorityQueue(n, vFences);
        while (!hQueue.isEmpty() && !vQueue.isEmpty()){
            if(hQueue.peek() == vQueue.peek()){
                return hQueue.peek() * hQueue.peek();
            }else if(hQueue.peek() < vQueue.peek()) {
                vQueue.poll();
            }else {
                hQueue.poll();
            }
    ...
    public PriorityQueue<Integer> getPriorityQueue(int size, int[] fences){
        Set<Integer> hSet = new HashSet<>();
        hSet.add(size - 1);
        for (int i = 0; i < fences.length; i++){
            hSet.add(fences[i] - 1);
            hSet.add(size - fences[i]);
            for (int j = 0; j < i; j++){
                hSet.add(Math.abs(fences[i] - fences[j]));
            }
        }
    ...
    public long minimumCost(String source, String target, char[] original, char[] changed, int[] cost) {
        ...
        for(int i = 0; i < source.length(); i++){
            sum += dp[source.charAt(i) - 'a'][target.charAt(i) - 'a'];
        }
        return sum;
    }

    public void refreshDp(int x, int y, int[][] dp){
        for (int j = 0; j < 26; j++){
            if(dp[x][y] + dp[y][j] < dp[x][j]){
                dp[x][j] = dp[x][y] + dp[y][j];
                refreshDp(x, j, dp);
            }
        }
    }

    private static void testMinimumCost() {
        String source = "abcd";
        String target = "acbe";
        ...
        Week20231224 week20231224 = new Week20231224();
        System.out.println(week20231224.minimumCost(source, target, original, changed, cost));
    }
